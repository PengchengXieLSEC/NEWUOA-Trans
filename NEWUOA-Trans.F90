!! NEWUOA-Trans considers solving the derivative-free optimization problems 
!! with transformed objective function (DFOTO problems).
!! NEWUOA-Trans is based on the NEWUOA-Fortran77 code 
!! of the late Professor Michael J. D. Powell
...
contains
        ...
        SUBROUTINE newuoa_min(CALFUN,CALGREY,NF, X,INFO, RHOBEG, RHOEND, IPRINT, MAXFUN,PROBLEM,FLAG_Epsilonk)
                implicit none
                interface
                        subroutine CALFUN(i_x, o_f,i_problem,i_info)
                                real(kind=8), dimension(:) :: i_x
                                real(kind=8) :: o_f
                                character(len=15) :: i_problem
                                integer(kind=4) :: i_info
                        end subroutine

                        subroutine CALGREY(i_x,i_k,o_g,i_flag,i_eps,i_eps1,i_eps2,i_rand,i_problem,&
                                        i_flag_epsilonk,i_info)
                                real(kind=8), dimension(:) :: i_x
                                real(kind=8) :: o_g
                                integer(kind=4) :: i_k
                ...
                call NEWUOA( CALFUN,CALGREY,NF,N,NPT,X,INFO,RHOBEG,RHOEND, &
        ...
        SUBROUTINE NEWUOA(CALFUN,CALGREY,NF,N,NPT,X,INFO,RHOBEG,RHOEND,IPRINT,MAXFUN,W,PROBLEM,FLAG_Epsilonk)
                implicit none
                ...
                integer :: NP, NPTM, NDIM, IXB, IXO, IXN, IXP, IFV, IGQ, &
                        IHQ, IPQ, IBMAT, IZMAT, ID, IVL, IW, INFO
                character(len=15) :: PROBLEM
                ...
                CALL NEWUOB(CALFUN,CALGREY,NF, N,NPT,X,INFO,RHOBEG,RHOEND,IPRINT,MAXFUN,W(IXB),  &
        ...
        SUBROUTINE NEWUOB(CALFUN,CALGREY,NF,N,NPT,X,INFO,RHOBEG,RHOEND,IPRINT,MAXFUN,XBASE, &
                ...
                logical :: EXIST
                real*8,allocatable :: EPS(:),EPS1(:),EPS2(:),RAND(:)


                !! Define the noise for the transformation
                ALLOCATE(EPS(MAXFUN))
                call random_seed
                ...
                310 IF (NF .GT. NFTEST) THEN
                       ...
                       !           ' called MAXFUN times.')
                        GOTO 530
                END IF
                CALL CALFUN(X,F,PROBLEM,INFO)
                
                IF (NF .GE. NPT+1) THEN
                        CALL CALGREY(X,NF-1,G2,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk,INFO)
                       ...
                       ! CALL evalfun(PROBLEM,N,X,G,INFO)
                        ...
                        !! Contain some synchronization of the function value 
                        !! at the optimal/initial interpolation points
                        ...
                        IF (NF .GE. NPT+2) THEN
                                ...
                                CALL CALGREY(XBEG,NF-2,G1,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk,INFO)
                                CALL CALGREY(XBEG,NF-1,G2,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk,INFO)
                                ...
                                END DO
                                CALL CALGREY(XTEMP,NF-2,G1,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk,INFO)
                                CALL CALGREY(XTEMP,NF-1,G2,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk,INFO)
                        ...
                        END IF 
                ELSE
                       CALL CALGREY(X,NPT,G1,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk,INFO)
                        F=F+G1
                ...
                IF (NF .GE. NPT+1) THEN 
                       CALL CALGREY(XTEMP,NF,G2,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk,INFO)
                       CALL CALGREY(XTEMP,NF-1,G1,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk,INFO)

                        DIFF=F-FOPT+(G2-G1)-VQUAD
                END IF

                ! DIFF changede beacause of the transformation
...
!! When vector r changes (that is, when DIFF changes), 
!! PQ and gradient also need to be adjusted accordingly.
...
PROGRAM test_newuoa
        ...
        DO IP=1,94
            ...
            NF=0
            INFO=IP
            CALL newuoa_min(CALFUN,CALGREY,NF,X,INFO, RHOBEG=1.0D0, RHOEND=1.0D-6, IPRINT=3, &
                MAXFUN=10000,PROBLEM=a(IP),FLAG_Epsilonk=2)
...
subroutine CALFUN(X,F,PROBLEM,INFO)
        integer*4 :: N,INFO,I
        real*8 :: X(:),F,G
        character(len=15) :: PROBLEM
        N=size(X)
...
subroutine CALGREY(X,K,G,FLAG,EPS,EPS1,EPS2,RAND,PROBLEM,FLAG_Epsilonk,INFO)
       ...
       ! CALL evalfun(PROBLEM,N,X,G,INFO)
        G=0.0D0
        IF (FLAG_Epsilonk .EQ. 0)THEN
            G=0
            CALL evalfun(PROBLEM,N,X,G,INFO)
        END IF

        IF (FLAG_Epsilonk .EQ. 1)THEN
            N=SIZE(X)
        ...
        IF (FLAG_Epsilonk .EQ. 2)THEN
            N=SIZE(X)
  
            IF (K .GT. FLAG)THEN
                CALL Laplace(100.0D0/K,Epsilonk)
                EPS1(K)=Epsilonk*10.0D0
...
!
      ...
      ,81,82,83,84,85,86,87,88,89,90,91,92,93,94)info
...
! Problem 32, also CUTEr.
      ...
      END IF
      return
     2 IF (trim(ful) .EQ. 'arglina4') THEN
...
! 61. But the paper is not available for now.
      ...
      END IF
      return
     8 IF (trim(ful) .EQ. 'bdqrticp') THEN
! See Powell, 2003, On trust region methods for unconstrained
! minimization without derivatives, Problem 2. Also see CUTEr.
! NOTICE: the definitions in the above references are different,
...
! Problem 28, also CUTEr.
      ...
      END IF
      return
     10 IF (trim(ful) .EQ. 'brownal') THEN
! See J. More, 1981, Testing Unconstrained Optimization Software,
! Problem 27, also CUTEr.
      ...
      END IF
      return
     11 IF (trim(ful) .EQ. 'broydn3d') THEN
! See CUTEr.
          f = 0.0D0
          DO i = 1, n
      ...
      END IF
      return
     12 IF (trim(ful) .EQ. 'broydn7d') THEN
...
! Problem 31, also CUTEr.
     ...
     14 IF (trim(ful) .EQ. 'chainwoo') THEN
! See CUTEr.
          f = 1.0D0
          DO i = 1, n/2-1
              j = 2*i
      ...
      END IF
      return
     15 IF (trim(ful) .EQ. 'chebquad') THEN
! See J. More, 1981, Testing Unconstrained Optimization Software,
! Problem 35, also CUTEr.
      ...
      END IF
      return
     17 IF (trim(ful) .EQ. 'chpowellb') THEN
...
! Problem 3.
      ...
      END IF
      return
     18 IF (trim(ful) .EQ. 'chpowells') THEN
...
! equality constrained optimization.
      ...
      END IF
      return
     16 IF (trim(ful) .EQ. 'chnrosnb') THEN
...
! ROSENBROCK. Compare with CHROSEN and ROSENBROCK.
      ...
      END IF
      return
     19 IF (trim(ful) .EQ. 'chrosen') THEN
          f = 0.0D0
          DO i = 1,n-1
...
! See CUTEr.
      ...
      END IF
      return
     21 IF (trim(ful) .EQ. 'cragglvy') THEN
! See CUTEr.
          f = 0.0D0
          DO j = 1, (n-2)/2
              i = 2*j
...
! See CUTEr.
     ...
     23 aaaaaa=1
     24 aaaaaa=1
     25 IF (trim(ful) .EQ. 'curly10' .or. trim(ful) .EQ. 'curly20' &
...
! See CUTEr.
     ...
     35 aaaaaa=1
     36 aaaaaa=1
     37 IF (trim(ful) .EQ. 'dixmaane' .or. trim(ful) .EQ. 'dixmaanf' &
...
! See CUTEr.
     ...
     39 IF (trim(ful) .EQ. 'edensch') THEN
! See CUTEr.
          f = 16.0D0
          DO i = 1, n-1
...
! See CUTEr.
      ...
      END IF
      return
     41 IF (trim(ful) .EQ. 'engval1') THEN
! See CUTEr. Compare with ARWHEAD.
      ...
      END IF
      return
     42 IF (trim(ful) .EQ. 'errinros') THEN
! See CUTEr. MODIFIED by Luksan, Matonoha, Vlcek, "Modified CUTE Probelems for
! Unconstrained Optimzation." It is another version of chained
...
! See http://www.opt.uni-duesseldorf.de/~jarre/dot/f_cx.m
      ...
      END IF
      return
     44 IF (trim(ful) .EQ. 'extrosnb') THEN
...
! by Guangye Li (SIAM Journal on Numerical Analysis, 1988).
      ...
      END IF
      return
     47 IF (trim(ful) .EQ. 'fletcbv2') THEN
! See CUTEr.
          h = 1.0D0/dble(n+1)
          tmp1 = y(1)**2
          DO i = 1, n-1
      ...
      END IF
      return
     48 IF (trim(ful) .EQ. 'fletcbv3') THEN
! See CUTEr. MODIFIED by Luksan, Matonoha, Vlcek, "Modified CUTE Probelems for
! Unconstrained Optimzation."
      ...
      END IF
      return
     49 IF (trim(ful) .EQ. 'fletchcr') THEN
! See CUTEr.
          f = 0.0D0
          DO i = 1, n-1
...
! See CUTEr
      ...
      !END IF
      !return
     50 IF (trim(ful) .EQ. 'freuroth') THEN
          tmp1 = 0.0D0
          tmp2 = 0.0D0
          DO i = 1, n-1
     ...
     51 IF (trim(ful) .EQ. 'genbrown') THEN
...
! unconstrained and equality constrained optimization.
      ...
      END IF
      return
     52 IF (trim(ful) .EQ. 'genhumps') THEN
! See CUTEr.
          zeta = 2.0D0
          f = 0.0D0
          DO i = 1, n-1
      ...
      END IF
      return
     53 IF (trim(ful) .EQ. 'genrose') THEN
! See CUTEr.
...
! Unconstrained Optimzation."
      ...
      END IF
      return
     55 IF (trim(ful) .EQ. 'integreq') THEN
! See J. More, 1981, Testing Unconstrained Optimization Software,
! Problem 29 and also CUTEr.
     ...
     56 IF (trim(ful) .EQ. 'liarwhd') THEN
          f = 0.0D0
          DO i =1 ,n
      ...
      END IF
      return
     57 IF (trim(ful) .EQ. 'lilifun3') THEN
...
! regularized Newton Method for Convex Minimization, Problem 3.
      ...
      END IF
      return
     58 IF (trim(ful) .EQ. 'lilifun4') THEN
! See Ying-Jie Li, Dong-Hui Li, Truncated
! regularized Newton Method for Convex Minimization, Problem 4.
      ...
      return
     59 aaaaaa=1
     60 IF (trim(ful) .EQ. 'morebv' .or. trim(ful).EQ. 'morebvl') THEN
...
! Unconstrained Optimzation."
      ...
      END IF
      return
     63 IF(trim(ful) .EQ. 'noncvxu2') THEN
! See CUTEr.
          f = 0.0D0
          DO i = 1, n
              j = mod(3*i-2,n)
      ...
      END IF
      return
     64 IF(trim(ful) .EQ. 'noncvxun') THEN
! See CUTEr.
          f = 0.0D0
          DO i = 1, n
              j = mod(2*i-1,n)
...
! See CUTEr.
      ...
      END IF
      return
     66 IF (trim(ful) .EQ. 'nondquar') THEN
     ...
     67 IF (trim(ful) .EQ. 'penalty1') THEN
          dp1 = 0.0D0
          dp2 = 0.0D0
          DO i = 1, n
              dp1 = dp1 + (y(i)-1.0D0)**2
      ...
      END IF
      return
     68 IF (trim(ful) .EQ. 'penalty2') THEN
          f = 0.0D0
          tmp = 0.0D0
          DO i = 2,n
              f = f + &
      ...
      return
     69 aaaaaa=1
     70 IF (trim(ful) .EQ. 'penalty3' .or. trim(ful) .EQ. 'penalty3p') &
            THEN
          f = 0.0D0
          r = 0.0D0
          s = 0.0D0
...
!            section 8.
      ...
      END IF
      return
     71 IF (trim(ful) .EQ. 'powellsg') THEN
          f = 0.0D0
          DO i = 1, n/4
             j = 4*(i-1) + 1
...
! see cuter.
     ...
     73 IF (trim(ful) .EQ. 'rosenbrock') THEN
...
! except the order of the variables.
     ...
     74 aaaaaa=1
     75 IF (trim(ful) .EQ. 'sbrybnd' .or. trim(ful) .EQ. 'sbrybndl') &
            THEN
...
! makes the function unbounded from below.
      ...
      END IF
      return
     76 IF (trim(ful) .EQ. 'schmvett') THEN
! See CUTEr.
          f = 0.0D0
          DO i = 1, n -2
      ...
      return
     77 aaaaaa=1
     78 IF (trim(ful) .EQ. 'scosine' .or.trim(ful).EQ.'scosinel') THEN
! See CUTEr. And see Luksan, Matonoha, Vlcek, "Modified CUTE Probelems for
! Unconstrained Optimzation." The scaling parameter of Luksan is 6
! instead of 12.
...
! j=n-2 should be defined seperately as well.
      ...
      END IF
      return
     80 IF(trim(ful) .EQ. 'sinquad') THEN
! See CUTEr.
          f = (y(1)-1.0D0)**4
          DO i = 2, n-1
      ...
      !END IF
      !return
     81 IF (trim(ful) .EQ. 'sparsine') THEN
          f = 0.0D0
          DO i = 1, n
              j1 = mod(2*i-1,n)+1
              j2 = mod(3*i-1,n)+1
      ...
      END IF
      return
     82 IF (trim(ful) .EQ. 'sparsqur') THEN
          f = 0.0D0
          DO i = 1, n
              j1 = mod(2*i-1,n)+1
              j2 = mod(3*i-1,n)+1
      ...
      END IF
      return
     83 IF (trim(ful) .EQ. 'spmsrtls') THEN
          DO i = 1, n
              par(i) = sin(dble(i)**2)
          ENDDO
          m = (n+2)/3
      ...
      END IF
      return
     84 IF (trim(ful) .EQ. 'srosenbr') THEN
          f = 0.0D0
          DO i = 1, n/2
...
! See http://www.opt.uni-duesseldorf.de/~jarre/dot/f_stmod.m
     ...
     86 IF (trim(ful) .EQ. 'tointgss') THEN
          f = 0.0D0
          DO i = 1, n-2
      ...
      END IF
      return 
     87 IF (trim(ful) .EQ. 'tointtrig') THEN
! See Ph. L. Toint, "Some Numerical Results Using a Sparse Matrix
! Updating Formula in Unconstrained Optimization", Example 3.6.
      ...
      END IF
      return
     88 IF (trim(ful) .EQ. 'tquartic') THEN
          f = (y(1)-1.0D0)**2
          DO i = 1, n-1
              f = f + (y(1)**2-y(i+1)**2)**2
          ENDDO
      END IF
      return 
     89 IF (trim(ful) .EQ. 'trigsabs') THEN
...
! The random numbers are replaced by "irregular" numbers generated with trigonometrics.
      ...
      END IF
      return
     90 IF (trim(ful) .EQ. 'trigssqs') THEN
! See Powell, "The NEWUOA Software for unconstrained optimization without derivatives", 2004, section 8.
! The random numbers are replaced by "irregular" numbers generated with trigonometrics.
          DO i = 1, 2*n
              DO j = 1, n
      ...
      END IF
      return
     91 IF (trim(ful) .EQ. 'trirose1') THEN
...
! by Guangye Li (SIAM Journal on Numerical Analysis, 1988).
      ...
      END IF
      return
     92 IF (trim(ful) .EQ. 'trirose2') THEN
! Tri-Diagonal ROSENBROCK. The Jacobian of the corresponding nonlinear
